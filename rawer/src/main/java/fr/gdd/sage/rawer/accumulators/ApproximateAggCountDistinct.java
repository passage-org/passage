...
public class ApproximateAggCountDistinct<ID,VALUE> implements SagerAccumulator<ID, VALUE> {
    ...
    public void accumulate(BackendBindings<ID, VALUE> binding, FunctionEnv functionEnv) {
        ...
        for (Var v : vars) {
            right = OpJoin.create(
                    OpExtend.extend(OpTable.unit(), v, ExprUtils.parse(binding.get(v).getString())),
                    right);
        }
        ...
        CacheId<ID,VALUE> cache; // TODO TODO TODO 
        ...
        String fmuAsString = estimatedFmu.get(Var.alloc("count")).getString();
        ...
        sumOfInversedProbaOverFmu += inversedProba / actualFMU;
